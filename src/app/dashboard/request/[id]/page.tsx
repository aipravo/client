								...
								msg.files?.length ? (
									<div className="d-flex align-items-center gap-3 justify-content-end mb-2">
										{msg.files.map((file, idx) => (
											<span
												key={idx}
												data-url={`${process.env.NEXT_PUBLIC_SERVER_URL}/${file}`}
												onClick={() => handleOpenModal(`${process.env.NEXT_PUBLIC_SERVER_URL}/${file}`)}
							...
							{subscription.is_active
								...
								(
									<>
										...
										<p>3️⃣ Получаешь ещё <span className='text-accent'>3</span> попытки! 🎯</p>
										<h5 className=' text-center text-md-start'>Удачи! 🚀</h5>
									</>
								)
							}
